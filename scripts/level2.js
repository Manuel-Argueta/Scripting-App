...
function displayLevelMap() {
  ...
  for (let i = 0; i < levelMapW; i++) {
    for (let j = 0; j < levelMapH; j++) {
      ...
      if (restrictedWalls.includes(levelMap[i][j].id) == false) {
            if (possWallI == i || possWallJ == j) {
            levelMap[i][j].style["background-color"] = "red"
            }
      }
...
function checkIfCompleted() {
    ...
    if (levelMap[pepeCurrentI][pepeCurrentJ].id == "block19") {
...
function checkIfWallCollision() {
    let endLog = document.createElement("p");
    let userConsole = document.getElementById("console-output")
    if (levelMap[pepeCurrentI][pepeCurrentJ].style["background-color"] == "red") {
